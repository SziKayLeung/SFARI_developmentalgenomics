...
#Phenotype file
phenotype=read.csv("/gpfs/mrc0/projects/Research_Project-MRC148213/sl693/RBFetal/00_metadata/WholeTargetedphenotype_fixed.csv")
...
fwrite(deseq_output$res_Wald, file="output_targeted_group_removinggt23pcw.csv")
fwrite(deseq_output$res_Wald[which(deseq_output$res_Wald$padj<0.05),], file="filtered_output_targeted_group_removinggt23pcw.csv")

fwrite(deseq_output$norm,file="output_norm_targeted_group_removinggt23pcw.csv")
fwrite(deseq_output$norm[which(deseq_output$norm$isoform%in%deseq_output$res_Wald[which(deseq_output$res_Wald$padj<0.05),]$isoform),],file="filtered_output_norm_targeted_group_removinggt23pcw.csv")
...
fwrite(deseq_output$anno_res,file="anno_targeted_group_removinggt23pcw.csv", quote=F)
...
fwrite(SFARI_hits,file="SFARI_hits_group_removinggt23pcw.csv", quote=F)
