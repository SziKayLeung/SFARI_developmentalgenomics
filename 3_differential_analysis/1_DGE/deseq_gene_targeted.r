...
fwrite(deseq_output$res_Wald[which(deseq_output$res_Wald$padj<0.05),], file="filtered_output_targeted_gene_group_removinglateprenatal.csv")
fwrite(deseq_output$res_Wald[which(deseq_output$res_Wald$pvalue<0.05),], file="nominal_output_targeted_gene_group_removinglateprenatal.csv")

fwrite(deseq_output$norm,file="output_norm_targeted_gene_group_removinglateprenatal.csv")
fwrite(deseq_output$norm[which(deseq_output$norm$isoform%in%deseq_output$res_Wald[which(deseq_output$res_Wald$padj<0.05),]$isoform),],file="filtered_output_norm_targeted_gene_group_removinglateprenatal.csv")
fwrite(deseq_output$norm[which(deseq_output$norm$isoform%in%deseq_output$res_Wald[which(deseq_output$res_Wald$pvalue<0.05),]$isoform),],file="nominal_output_norm_targeted_gene_group_removinglateprenatal.csv")
...
fwrite(deseq_output$anno_res,file="anno_targeted_gene_group_removinglateprenatal.csv", quote=F)
...
fwrite(SFARI_hits,file="SFARI_hits_targeted_gene_group_removinglateprenatal.csv", quote=F)
