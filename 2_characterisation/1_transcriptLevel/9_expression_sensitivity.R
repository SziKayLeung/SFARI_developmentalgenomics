...
ggplot(dat2, aes(x = as.factor(whole_nsamples), y = proportion, colour = cate_sumFL, group = cate_sumFL)) +
  ...
  geom_line() + scale_y_log10(
    labels = percent_format(accuracy = 1)  # keeps labels as percentages
  ) + 
...
# output
write.table(as.data.frame(dat2), "/lustre/projects/Research_Project-MRC190311/longReadSeq/ONTRNA/SFARI/C_Whole_Targeted/9_sqanti_final/sensitivity_expression_threshold.txt", sep = "\t", quote = F)
