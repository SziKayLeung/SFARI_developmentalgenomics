...
## using K.Chundruv final dataset classification txt with monoexonic genomic intron removed
## ---------------------------------
...
whole.class.files <- class.files %>% filter(whole_nsamples >= 2, whole_nreads >= 2)
write.table(whole.class.files$isoform, paste0(dirnames$sqanti,"sqantifiltered_monoexonic_2reads2samplesfiltered_whole_intergenicGenicIntron_ID.txt"), quote=F, sep = "\t", row.names= F, col.names = F)
